...
export function ProgramBlendStep({
  ...
  const totalPercent = programSelections.reduce((sum, entry) => sum + entry.weight, 0);

  return (
    <section className="space-y-4" data-testid="onboarding-step-program-blend">
      <div>
        <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Step 5 of 6</p>
        <h2 className="mt-2 text-xl font-semibold text-slate-100">Program Blend</h2>
        <p className="mt-1 text-sm text-slate-400">
          Assign percentage weights to each program. Weights can be 0% if not used.
        </p>
      </div>

      <p className="text-sm text-slate-400">Total blend: {totalPercent.toFixed(0)}%</p>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-200">{weight.toFixed(0)}%</span>
                </div>
                <p className="mt-1 text-xs uppercase tracking-[0.15em] text-slate-500">
                  {program.default_days_per_week} days per week
                </p>
                <p className="mt-2 text-sm text-slate-400">
                  Focus: {program.muscleCoverage.slice(0, 3).map((entry) => formatMuscleLabel(entry.muscle)).join(", ") || "General"}
                </p>
                <label className="mt-3 block">
                  ...
                  <input
                    ...
                    value={weight}
                    onChange={(event) =>
                      onWeightChange(program.id, clampPercent(Number(event.target.value)))
                    }
